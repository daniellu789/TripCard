...
public class FavoriteMealAdapter extends ParseQueryAdapter<Card> {
	...
	public View getItemView(Card card, View v, ViewGroup parent) {
		...
		if (v == null) {
			v = View.inflate(getContext(), com.levart.mealspotting.R.layout.item_list_favorites, null);
		}

		super.getItemView(card, v, parent);

		ParseImageView mealImage = (ParseImageView) v.findViewById(com.levart.mealspotting.R.id.icon);
		...
		TextView titleTextView = (TextView) v.findViewById(com.levart.mealspotting.R.id.text1);
		titleTextView.setText(card.getTitle());
		TextView ratingTextView = (TextView) v
				.findViewById(com.levart.mealspotting.R.id.favorite_meal_rating);
